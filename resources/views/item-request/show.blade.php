                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($itemRequest->item->stock < $itemRequest->item->min_stock)
                                                <span class="badge badge-danger">{{ $itemRequest->item->stock }} (Low
                                                    Stock)</span>
                                            @else
                        ...
                        <div class="row mt-4">
                            <div class="col-12">
                                ...
                                @if (auth()->user()->hasRole(['super admin', 'admin']))
                                    @if ($itemRequest->status == 'pending')
                                        <form action="{{ route('item-requests.approve', $itemRequest) }}" method="POST"
                                            class="d-inline" data-confirm="Approve this item request?">
                                            @csrf
                                            @method('PUT')
                                ...
                                @else
                                    @if ($itemRequest->status == 'pending' && $itemRequest->user_id == auth()->id())
                                        <form action="{{ route('item-requests.destroy', $itemRequest) }}" method="POST"
                                            class="d-inline" data-confirm="Cancel this item request?">
                                            @csrf
                                            @method('DELETE')
